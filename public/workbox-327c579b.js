define(["exports"], function (exports) {
    ...
    */
    const generatorFunction = (code, details = {}) => {
        ...
        if (!message) {
            throw new Error(
                `Unable to find message for code '${code}'.`,
            );
        }
            ...
            if (!handler) {
                {
                    ...
                    logger.debug(
                        `No route found for: ${getFriendlyURL(url)}`,
                    );
                }
                return;
            }
    ...
    */
    ...
    const cacheNames = {
        ...
        getPrecacheName: (userCacheName) => {
            return (
                userCacheName ||
                _createCacheName(_cacheNameDetails.precache)
            );
        },
        getPrefix: () => {
        ...
        getRuntimeName: (userCacheName) => {
            return (
                userCacheName ||
                _createCacheName(_cacheNameDetails.runtime)
            );
        },
        getSuffix: () => {
    ...
    */
    ...
    async function cacheMatchIgnoreParams(
        ...
        const strippedRequestURL = stripParams(
            request.url,
            ignoreParams,
        );
    ...
    class StrategyHandler {
        ...
        async fetch(input) {
            ...
            const originalRequest = this.hasCallback("fetchDidFail")
                ...
                : null;
            try {
                for (const cb of this.iterateCallbacks(
                    "requestWillFetch",
                )) {
                    request = await cb({
    ...
    */
    ...
    class Strategy {
        ...
        constructor(options = {}) {
             ...
             * @type {string}
             */
            this.cacheName = cacheNames.getRuntimeName(
                options.cacheName,
            );
            /**
             * The list
        ...
        handleAll(options) {
            ...
            const responseDone = this._getResponse(
                handler,
                request,
                event,
            );
